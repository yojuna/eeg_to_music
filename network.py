...
class Model(nn.Module):
    ...
    def forward(self, eeg_array, mel_input, pos_eeg_signal, pos_mel):
...
class Encoder(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        print('encprenet out shape: ', x.shape)

        # Get positional embedding, apply alpha and add
        pos = self.pos_emb(pos)
        print('pos emb shape: ', pos.shape)
        x = pos * self.alpha + x
        print('pos emb add shape: ', x.shape)

        # Positional dropout
        x = self.pos_dropout(x)
        print('after pos dropout shape: ', x.shape)
        ...
        for layer, ffn in zip(self.layers, self.ffns):
            ...
            if x.shape[1] == c_mask.shape[1]:
                x, attn = layer(x, x, mask=mask, query_mask=c_mask)
                print('x; attn shapes', x.shape, attn.shape)
                x = ffn(x)
                attns.append(attn)
            else:
                print('yeh locha tha: ', x.shape[1], ' =/= ', c_mask.shape[1])
